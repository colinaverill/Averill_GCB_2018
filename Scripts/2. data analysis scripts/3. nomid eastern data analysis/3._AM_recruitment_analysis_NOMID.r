...
#This script fits AM recruitment for all plots.
...
#Save paths
model.path      = remote_AM_recruitment_model_path.nomid
prediction.path = remote_AM_recruitment_prediction_path.nomid
summary.path    = remote_AM_recruitment_summary_path.nomid
filtered.path   = remote_AM_recruitment_filtered_path.nomid
range.path      = remote_AM_recruitment_range_path.nomid
...
dat <- d[,.(recruit.adult.am, pH_H2O, cn, mat, map, relEM, relAM, tot.15, clay, REMPER,PLT_CN, PREV_PLT_CN, BASAL)]
dat <- dat[complete.cases(dat),]
#save for later
saveRDS(dat, filtered.path)

#grab variables for model
y  <- dat$recruit.adult.am
t  <- dat$REMPER
x1 <- dat$pH_H2O
x2 <- dat$cn
x3 <- dat$mat
...
#This also uses the probability of recruitment, pre-bernoulli transform, to avoid non-linear confidence intervals.
for (i in 1:N.pred){
