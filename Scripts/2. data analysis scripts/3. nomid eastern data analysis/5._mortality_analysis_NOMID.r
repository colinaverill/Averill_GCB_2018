...
#specify filepaths
model.path      = remote_mortality_model_path.nomid
prediction.path = remote_mortality_prediction_path.nomid
summary.path    = remote_mortality_summary_path.nomid
filtered.path   = remote_mortality_filtered_path.nomid
range.path      = remote_mortality_range_path.nomid


#load data - subset 
#d <- readRDS('/fs/data3/caverill/FIA7_Ndep_data.processed/Product_2.rds')
d <- readRDS(remote_Product_2.E_nomid.path)
