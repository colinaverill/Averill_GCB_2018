...
#save paths.
model.path      = remote_growth_model_path.nomid
prediction.path = remote_growth_prediction_path.nomid
summary.path    = remote_growth_summary_path.nomid
filtered.path   = remote_growth_filtered_path.nomid
range.path      = remote_growth_range_path.nomid


#load data - local path
#d<- readRDS('/fs/data3/caverill/FIA7_Ndep_data.processed/Product_3.rds')
#load data - remote path
d <- readRDS(remote_Product_3.E_nomid.path)
...
y.hat[i] <- a + b*x1[i] + c*x2[i] + d*x3[i] + e*x4[i] + f*x5[i] + g*x6[i] + h*x7[i] + j*x6[i]*x7[i] + k*x8[i]
}

#predictions
for (i in 1:N.pred){
y.x1[i] <- a + b*x1.range[i] + c*m.x2 + d*m.x3 + e*m.x4 + f*m.x5 + g*m.x6 + h*m.x7 + j*m.x6*m.x7 + k*m.x8
y.x2[i] <- a + b*m.x1 + c*x2.range[i] + d*m.x3 + e*m.x4 + f*m.x5 + g*m.x6 + h*m.x7 + j*m.x6*m.x7 + k*m.x8
y.x3[i] <- a + b*m.x1 + c*m.x2 + d*x3.range[i] + e*m.x4 + f*m.x5 + g*m.x6 + h*m.x7 + j*m.x6*m.x7 + k*m.x8
y.x4[i] <- a + b*m.x1 + c*m.x2 + d*m.x3 + e*x4.range[i] + f*m.x5 + g*m.x6 + h*m.x7 + j*m.x6*m.x7 + k*m.x8
y.x5[i] <- a + b*m.x1 + c*m.x2 + d*m.x3 + e*m.x4 + f*x5.range[i] + g*m.x6 + h*m.x7 + j*m.x6*m.x7 + k*m.x8
y.x6[i] <- a + b*m.x1 + c*m.x2 + d*m.x3 + e*m.x4 + f*m.x5 + g*x6.range[i] + h*m.x7 + j*x6.range[i]*m.x7 + k*m.x8
y.x7[i] <- a + b*m.x1 + c*m.x2 + d*m.x3 + e*m.x4 + f*m.x5 + g*m.x6 + h*x7.range[i] + j*m.x6*x7.range[i] + k*m.x8
y.x7.am[i] <- a + b*m.x1 + c*m.x2 + d*m.x3 + e*m.x4 + f*m.x5 + g*0 + h*x7.range[i] + j*0*x7.range[i] + k*m.x8
y.x7.em[i] <- a + b*m.x1 + c*m.x2 + d*m.x3 + e*m.x4 + f*m.x5 + g*1 + h*x7.range[i] + j*1*x7.range[i] + k*m.x8
y.x8[i] <- a + b*m.x1 + c*m.x2 + d*m.x3 + e*m.x4 + f*m.x5 + g*m.x6 + h*m.x7 + j*m.x6*m.x7 + k*x8.range[i]
}
