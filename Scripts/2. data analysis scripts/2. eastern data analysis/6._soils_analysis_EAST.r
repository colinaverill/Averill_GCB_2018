#building bayesian analysis of soil C. Linear model accounting for increasing variance in C as a function of its observed value. 
...
#load data - remote
d<- readRDS(remote_Product_1.E.path)

#specify file paths
model.path      = remote_soils_model_path.east
prediction.path = remote_soils_prediction_path.east
summary.path    = remote_soils_summary_path.east
filtered.path   = remote_soils_filtered_path.east
range.path      = remote_soils_range_path.east


#Convert C and N storage from kg / m2 to g /m2 for easier interpretation of log transformation.
d[,C.storage := C.storage * 1000]
d[,N.storage := N.storage * 1000]

#gather complete cases- everything that is used to model C.storage.
#complete cases drops us from 3621 observations to 2880. 
dat <-   d[,.(mat, map, pH_H2O, tot.15, relEM, clay, N.storage, C.storage)]
