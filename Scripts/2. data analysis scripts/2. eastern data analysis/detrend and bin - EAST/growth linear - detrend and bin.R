...
#Note- this analysis shows no need for agaussian perturbation to capture growth.
rm(list=ls())
library(data.table)
library(runjags)
library(wesanderson)
...
#specify paths for the Ndep prediction, the data, and the model parameter estimates. 
  fit.path    <- '/fs/data3/caverill/FIA7_Ndep_data.processed/east.data_analysis_output/FIA7_051517_growth.summary.Rdata'
range.path    <- '/fs/data3/caverill/FIA7_Ndep_data.processed/east.data_analysis_output/FIA7_051517_growth.predictor_ranges.rds'
filtered.path <- '/fs/data3/caverill/FIA7_Ndep_data.processed/east.data_analysis_output/FIA7_051517_growth.filtered.rds'
bins.am.path  <- '/fs/data3/caverill/FIA7_Ndep_data.processed/east.data_analysis_output/east_growth.am_bins.rds'
bins.em.path  <- '/fs/data3/caverill/FIA7_Ndep_data.processed/east.data_analysis_output/east_growth.em_bins.rds'
...
plot(bins.am$detrend ~ bins.am$tot.15, pch = 16,
     ylim=limy, xlim=c(min(d$tot.15),max(d$tot.15)))
