...
#load data.
raw   <- readRDS(local_soils_filtered_path)
d     <- readRDS(local_soils_summary_path)
bins  <- readRDS(bins_soils_all_path)
...
png(filename=Figure3_all_path,width=7,height=4.5,units='in',res=300)
...
mtext(expression(paste('1.2 kg N ha'^'-1',' yr'^'-1')), cex = 1.3, line = -1.1, adj = 0.95, side = 1)
#r2 label
r.sq <- rsq.table[analyses == 'soilC.lo',]$r.sq
mylabel = bquote(italic(R)^2 == .(format(r.sq, digits = 2)))
mtext(side=3, mylabel, cex = 1.3, line = -2, adj = 0.05)
...
mtext(expression(paste('17.8 kg N ha'^'-1',' yr'^'-1')), cex = 1.3, line = -1.1, adj = 0.95, side = 1)
#r2 label
