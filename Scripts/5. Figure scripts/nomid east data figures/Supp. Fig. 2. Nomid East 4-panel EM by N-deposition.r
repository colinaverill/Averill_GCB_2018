...
g.out    <- readRDS(local_growth.gaus_summary_path.nomid)
m.out    <- readRDS(local_mortality_summary_path.nomid)
r.out.am <- readRDS(local_AM_recruitment_summary_path.nomid)
r.out.em <- readRDS(local_EM_recruitment_summary_path.nomid)
b.out    <- readRDS(local_beta_summary_path.nomid)
save.path <- Supp_Figure2_all_path
...
range   <- readRDS('/fs/data3/caverill/FIA7_Ndep_data.processed/nomid_east.data_analysis_output/FIA7_051517_growth.predictor_ranges.rds')
g.range <- range$x7.range
range   <- readRDS('/fs/data3/caverill/FIA7_Ndep_data.processed/nomid_east.data_analysis_output/FIA7_NOMID_EAST_052617_runjags_NOMID_EAST_mortality.predictor_ranges.rds')
m.range <- range$range.x6
r.range <- readRDS('/fs/data3/caverill/FIA7_Ndep_data.processed/nomid_east.data_analysis_output/FIA7_NOMID_EAST_052617_AM.adult_linear_recruit.predictor_ranges.rds')
r.range <- r.range$x5.r
b.range <- readRDS('/fs/data3/caverill/FIA7_Ndep_data.processed/nomid_east.data_analysis_output/FIA7_nomid_east_053017_rel.abundance.ranges.rds')
b.range <- b.range$ndep.range


#save dimensions, destination
png(filename=save.path,width=7,height=7,units='in',res=300)
...
plot(b.mean ~ b.range, lwd=0, ylim= limy, cex.axis = cex.olab)
...
limy <- c(0,0.03)
#am line
plot(m.ndep[,4] ~ m.range, lwd=0, ylim= limy, cex.axis = cex.olab)
...
#limy <- c(1, 1.45)
#am line
plot(g.ndep.am[,4] ~ g.range, lwd=0, ylim= limy, cex.axis = cex.olab)
...
#legend
legend(5, 0.275,c('AM','EM'), lwd=2, col=c(cols[1],cols[5]), bty='n', y.intersp = 1, x.intersp = 0.75, cex=1.2, seg.len=1)
...
#am line
plot(r.ndep.am[,4] ~ r.range, lwd=0, ylim= limy, cex.axis = cex.olab)
...
mtext('Eastern U.S., no midwest'  , side=3, out=T, cex=1.5, line = -1)
