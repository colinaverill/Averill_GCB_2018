...
#specify paths for the Ndep prediction, the data, and the model parameter estimates. 
   model.path <- local_soils_model_path.east
     fit.path <- local_soils_summary_path.east
filtered.path <- local_soils_filtered_path.east
    bins.path <- bins_soils_east_path
...
lo.rsq <- summary(lm(bins.myc$detrend.lo ~ predict(lo.spline, bins.myc$relEM)$y))$r.squared
...
hi.rsq <- summary(lm(bins.myc$detrend.hi ~ predict(hi.spline, bins.myc$relEM)$y))$r.squared
...
out[analyses == 'soilC.lo',]$r.sq <- lo.rsq
out[analyses == 'soilC.hi',]$r.sq <- hi.rsq
