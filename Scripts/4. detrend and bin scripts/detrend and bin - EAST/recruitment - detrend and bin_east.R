...
#set paths.
   em.model.path <- local_EM_recruitment_model_path.east
   am.model.path <- local_AM_recruitment_model_path.east
     am.fit.path <- local_AM_recruitment_summary_path.east
     em.fit.path <- local_EM_recruitment_summary_path.east
      range.path <- local_AM_recruitment_range_path.east
am.filtered.path <- local_AM_recruitment_filtered_path.east
em.filtered.path <- local_EM_recruitment_filtered_path.east
       bins.path <- bins_recruitment_east_path

#load recruitment fits.
...
d.em <- d.em[!(PLT_CN == 168931301010661),]
d.em <- data.table(d.em)

#get binned Ndep categories
n.bins <- 8
d.am     <- d.am[order(tot.15),]
d.am$seq <- seq(1,nrow(d.am))
d.am$cat <- cut(d.am$seq,n.bins, labels=F)
d.em     <- d.em[order(tot.15),]
d.em$seq <- seq(1,nrow(d.em))
d.em$cat <- cut(d.em$seq,n.bins, labels=F)
...
plot(bins$am.detrend ~ bins$tot.15, pch = 16, 
     ylim=c(0,0.65), xlim=c(min(d.am$tot.15),max(d.am$tot.15)))
#error bars
...
plot(bins$em.detrend ~ bins$tot.15, pch = 16, 
     ylim=c(0,0.65), xlim=c(min(d.em$tot.15),max(d.em$tot.15)))
#error bars
...
am.rsq <- summary(lm(bins$am.detrend ~ predict(am.spline,bins$tot.15)$y))$r.squared

#mean EM fit R2 = 0.35
em.spline <- smooth.spline(r.ndep.em[,4] ~ r.range)
summary(lm(bins$em.detrend ~ predict(em.spline,bins$tot.15)$y))
em.rsq <- summary(lm(bins$em.detrend ~ predict(em.spline,bins$tot.15)$y))$r.squared

#update binned rsq table
out <- data.table(readRDS(east_binned_rsq_summary_path))

out[analyses == 'recruitment.AM',]$r.sq <- am.rsq
out[analyses == 'recruitment.EM',]$r.sq <- em.rsq

saveRDS(out, east_binned_rsq_summary_path)