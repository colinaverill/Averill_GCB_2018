...
#specify paths.
   model.path <- local_mortality_model_path.east
     fit.path <- local_mortality_summary_path.east
   range.path <- local_mortality_range_path.east
filtered.path <- local_mortality_filtered_path.east
    bins.path <- bins_mortality_east_path
...
plot(bins$detrend.am ~ bins$tot.15, pch = 16, 
     ylim=c(0,0.035), xlim=c(min(d$tot.15),max(d$tot.15)))
...
#This gives you the R2 of the binned means - 0.67!
thing <- smooth.spline(inv.logit(m.ndep[,1]) ~ m.range)
summary(lm(bins$detrend ~ predict(thing, bins$tot.15)$y))
mort.rsq <- summary(lm(bins$detrend ~ predict(thing, bins$tot.15)$y))$r.squared
  
#update binned rsq table
out <- data.table(readRDS(east_binned_rsq_summary_path))

out[analyses == 'mortality',]$r.sq <- mort.rsq
