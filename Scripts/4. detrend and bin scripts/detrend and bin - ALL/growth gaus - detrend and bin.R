...
#specify paths for the Ndep prediction, the data, and the model parameter estimates.
   model.path <- local_growth.gaus_model_path
     fit.path <- local_growth.gaus_summary_path
   range.path <- local_growth.gaus_range_path
filtered.path <- local_growth.gaus_filtered_path
    pred.path <- local_growth.gaus_prediction_path
 bins.am.path <- bins_growth.gaus_all_AM_path
...
#this is based on visual inspection of model chains, selecting chains that do converge.
pred.model <-readRDS(pred.path)
pred.trim <- as.mcmc.list(pred.model)
pred.trim <- mcmc.list(pred.trim[[10]],pred.trim[[3]], pred.trim[[2]])
