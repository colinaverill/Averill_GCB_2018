...
#beta coefficient defined as the abosolute value of change in dependent variable in response to an increase in 1SD of a predictor
#relative to the value of the dependent variable at the median value of the predictor holding all other predictors constant at their means.
#Also grabbing raw parameter values and "p-values" whether or not 95% CI overlaps 0.
...
output.list <- list()
output.path <- all_beta_path

####SOIL C BETA FACTORS####
soil.data  <- readRDS(local_soils_filtered_path)
soil.pred  <- readRDS(local_soils_summary_path)
pred.range <- readRDS(local_soils_range_path)
...
relEM.data  <- readRDS(local_beta_filtered_path)
relEM.pred  <- readRDS(local_beta_summary_path)
pred.range  <- readRDS(local_beta_range_path)
...
####GROWTH BETA FACTORS####
#grab growth data
growth.data  <- readRDS(local_growth.gaus_filtered_path)
growth.pred  <- readRDS(local_growth.gaus_summary_path)
pred.range   <- readRDS(local_growth.gaus_range_path)
...
####AM RECRUITMENT####
recruitAM.data  <- readRDS(local_AM_recruitment_filtered_path)
recruitAM.pred  <- readRDS(local_AM_recruitment_summary_path)
pred.range      <- readRDS(local_AM_recruitment_range_path)
...
####EM RECRUITMENT####
recruitEM.data  <- readRDS(local_EM_recruitment_filtered_path)
recruitEM.pred  <- readRDS(local_EM_recruitment_summary_path)
pred.range      <- readRDS(local_EM_recruitment_range_path)
...
mortality.data  <- readRDS(local_mortality_filtered_path)
mortality.pred  <- readRDS(local_mortality_summary_path)
pred.range      <- readRDS(local_mortality_range_path)
